...
bitstream = "../bof/adc_receiver_v0/adc_receiver_v0_2024_Jan_20_1153.fpg"
...
class NRT_channelizer(object):
  def __init__(self, name, bitstream=None, Fe=None, feed='BF'):
    ...
    self.Receivers = [channelizer.receiver(fpga=self.fpga, Fe=self.Fe, decimation=2, receiver_basename="receiver0_"),
                      channelizer.receiver(fpga=self.fpga, Fe=self.Fe, decimation=2, receiver_basename="receiver1_"),
                      ]
  ...
  def conf_arb_gen(self, **kwargs):
    ...
    if kwargs["mode"] == "constant":
      ...
      assert "amp" in kwargs, "amp kwarg must be specified for mode \"sine\""
      ...
      data = kwargs['amp'] * np.sin(2*np.pi * k0 * np.arange(nof_samples) / nof_samples)
      data = data.astype(dt)
    else:
...
receiver_configs = [
                    ...
                    {"Fc":1.425e9, "decimation": 8, "dst_IP": 0xc0a805b4, "dsp_port": 0xce01, },
...
#mydesign.conf_arb_gen(mode="sine", amp=10, f0=50e6)
...
for receiver in mydesign.Receivers:
  ...
  #snap_name, decimation, ymin, ymax = "decim_out", receiver.decimation, -10, 20
  if decimation == 8:
    ymin, ymax = -35, -20
  ...
  med = 10*np.log10(np.median(DATs.flatten()))
  mad = np.median(np.abs(10*np.log10(DATs)-med).flatten())
  vmin = med - 3 * mad
  vmax = med + 3 * mad
